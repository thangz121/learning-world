...
public class MarketBootstrap : MonoBehaviour {
  ...
  static string BallPraiseText { get { return DialogueLang.T("Great! Ball!", "Gi?i! B�ng!"); } } // manifest ok_05
...
// B presenters (Unity instantiates via AddComponent; Bind injects services).
    ...
    try {
      ...
      dialog.Build(_hud);
      // S3-P2Z6: the chooser auto-closes once the child walks off to play (it
      // used to float over the zone panel and ate the Play button).
      dialog.Player = _builder != null && _builder.Player != null
        ? _builder.Player.transform : null;
    ...
    if (_hud != null) _hud.ShowObjective(MarketBuilder.HubSelectionOnly
      ? DialogueLang.T("Choose a gate!", "Ch?n m?t c?ng nh�!")
      : DialogueLang.T("Talk to Milo", "N�i chuy?n v?i Milo"));
    if (!MarketBuilder.HubSelectionOnly) {
  ...
  void OnWorldChanged(WorldChangedEvent e) {
    ...
    if (to != null) {
      ...
      if (_hud != null) {
        ...
        // Vietnamese mode: the subject name alone ("To�n") � English mode keeps
  ...
  async void TravelToSubjectAsync(SubjectDefinition to) {
    ...
    try {
      ...
      if (_hud != null) { try { _hud.ShowObjective(DialogueLang.T("Entering ", "�ang v�o ") + to.DisplayName + "�"); } catch (System.Exception) { } }
