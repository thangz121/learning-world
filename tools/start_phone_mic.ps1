# tools/start_phone_mic.ps1 — 1-click launcher for the phone-mic gateway.
#
...
#   [3] starts the gateway (STEP 1-5 logs + QR PNG saved + served at /qr.png),
#   [4] opens the PC browser at https://127.0.0.1:8443/qr showing the BIG QR,
#   [5] you scan that QR with the phone camera -> page opens, no typing.
#
# Then on the phone: START > allow mic > speak > STOP.
# Watch EITHER side to see how far it got:
#   PC window: [STEP 1/9]..[STEP 9/9]   Phone page: STEP 1/5..5/5 + steps log.
#
...
Write-Host "[3/3] starting gateway — keep this window open (STEP logs appear here)..." -ForegroundColor Green
...
Start-Process "https://127.0.0.1:$HttpsPort/qr"
